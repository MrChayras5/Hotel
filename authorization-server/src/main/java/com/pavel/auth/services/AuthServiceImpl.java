...
public class AuthServiceImpl implements AuthService {
	...
	public TokenResponse autenticar(LoginRequest request) throws Exception {
		...
		if (userDetails == null
				|| !new BCryptPasswordEncoder().matches(request.password(), userDetails.getPassword())) {
			throw new IllegalArgumentException("Credenciales inválidas");
		}

		Instant now = Instant.now();

		JWTClaimsSet claimsSet = new JWTClaimsSet.Builder().issuer("http://localhost:9000")
				.subject(userDetails.getUsername()).issueTime(Date.from(now))
				.expirationTime(Date.from(now.plusSeconds(1800))).jwtID(UUID.randomUUID().toString())
				.claim("roles",
