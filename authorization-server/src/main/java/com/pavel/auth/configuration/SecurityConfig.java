package com.pavel.auth.configuration;
...
import com.pavel.auth.services.CustomUserDetails;
...
public class SecurityConfig {
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests((authorize) -> authorize.requestMatchers("/api/login").permitAll()
	...
	CommandLineRunner initData(UsuarioRepository userRepo, RolRepository rolRepo, PasswordEncoder encoder) {
		return args -> {
			Rol adminRole = rolRepo.findByNombre("ROLE_ADMIN").orElseGet(() -> {
				Rol r = new Rol();
				r.setNombre("ROLE_ADMIN");
				return rolRepo.save(r);
			});

			Rol userRole = rolRepo.findByNombre("ROLE_USER").orElseGet(() -> {
				Rol r = new Rol();
